	...
	GetHRTF(1,data->ch[1].h_swap,triplet.polar[2].azimuth,triplet.polar[2].elevation);
   	for(int n=0;n<HRTFLEN*2;++n){
		...
		if(!data->ch[0].isFirstBuf){
			UnityComplexNumber::Scale<float,double,float>(data->ch[0].h_now[n],0.001,data->ch[0].h_now[n]);
			UnityComplexNumber::Scale<float,double,float>(data->ch[1].h_now[n],0.001,data->ch[1].h_now[n]);
			UnityComplexNumber::Scale<float,double,float>(data->ch[0].h_prev[n],0.999,data->ch[0].h_prev[n]);
			UnityComplexNumber::Scale<float,double,float>(data->ch[1].h_prev[n],0.999,data->ch[1].h_prev[n]);
